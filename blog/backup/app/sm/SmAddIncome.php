...
class SmAddIncome extends Model
{
    ...
    public function IncomeACSubHead(){
        return $this->belongsTo('App\SmSubAccount','income_sub_head_id', 'id');
    }
    ...
    public static function monthlyExpense($i){
        ...
        $m_item_receives = SmItemReceive::where('active_status', 1)->where('received_date', 'like', date('Y-m-').$i)->sum('total_paid');
    ...
    public static function yearlyExpense($i){
        ...
        $m_item_receives = SmItemReceive::where('active_status', 1)->where('received_date', 'like', date('Y-'.$i).'%')->sum('total_paid');
