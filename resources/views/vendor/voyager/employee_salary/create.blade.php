...
@section('content')
    <div class="page-content edit-add container-fluid">
        <form action="{{url('admin/employee-salaries')}}" method="POST" enctype="multipart/form-data">
...
{{--                                    <input type="number" min="0"  required class="form-control" name="account_id" placeholder="">--}}
                                    <select type="number" required class="form-control select2" name="account_id">
                                        ...
                                        @foreach($accounts as $key=>$account)
                                            ...
                                            <option @if($request_data['account_id']==$account->id) selected @endif value="{{$account->id}}">{{$account->account_title}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group" >
                                    ...
                                    <select type="number" required class="form-control select2" name="employee_category_id">
                                        <option>Select One</option>
                                        @foreach($employee_categories as $key=>$employee_category)

                                                <option @if($request_data['employee_category_id']==$employee_category->id) selected @endif value="{{$employee_category->id}}">{{$employee_category->name}}</option>
        ...
        <div class="row">
            ...
            <div class="col-md-10">
                <table class="table table-hover" id="myTable">
                    ...
                    @if(@$employees_name)
                        @foreach(@$employees_name as $key=>$employee)
                    <tr>

                        <td>
                            <input type="text" name="employee_name[]" placeholder="" required class="form-control employee_name" value="{{$employee->name}}">
                        </td>
                        <td><input type="number" name="current_salary[]" placeholder="" required class="form-control current_salary" value="{{$employee->salary}}"></td>
                        <td><input type="text" name="absent[]" placeholder="" required class="form-control absent"></td>
                        <td><input type="number" name="actual_salary[]" readonly placeholder="" required class="form-control calculated_actual_salary"></td>
                        <td><input type="number" name="paid_salary[]" placeholder="" required class="form-control paid_salary"></td>
