        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <div class="panel panel-bordered">
                    <!-- form start -->
                    <form action="{{url('admin/vendor-letter-credits/'.$letter_credit->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <!-- Adding / Editing -->
                            ...
                            <div class="col-md-4" >
                                <div class="form-group" >
                                    ...
                                    <select disabled class="form-control select2 select-contact board " required name="po_id">

                                        <option readonly value="">Select One</option>
                                        @foreach($purchase_order as $order)
                                            <option  @if($letter_credit->po_id == $order->id)  selected @endif value="{{$order->id}}">{{$order->po_number}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group" >
                                    ...
                                    <input type="text" required class="form-control" value="{{$letter_credit->lc_number}}" name="lc_number" placeholder="LC Number">
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group" >
                                    ...
                                    <input type="text" readonly value="{{$letter_credit->lc_qty}}" required class="form-control lc_qty" name="lc_qty" >
                                </div>
                            </div>

                                    @if($letter_credit->return_po_id > 0)
                            <div class="col-md-4">
                                <div class="form-group" >
                                    <label class="font-weight-bold" for="name">Return Po<span class="color">*</span></label>
                                    <select  disabled class="form-control select2 select-contact board " required name="return_po">

                                        <option readonly value="">Select One</option>
                                        @foreach($return_po as $order)
                                            <option  @if($letter_credit->return_po_id == $order->id)  selected @endif value="{{$order->id}}">{{$order->po_number.'|'.$order->qty.' '.$order->unit->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                </div>
                            ...
                            <table class="table table-striped" id="myTable">
                                ...
                                <tbody id="acc_table">
{{--                                {{dd($letter_credit->blnumber)}}--}}
                                @foreach($letter_credit->blnumber as $key=>  $bl)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-group" >
                                            ...
                                            <input class="form-control" value="{{$bl->bl_number}}" required type="text" name="bl_number[]">
                                            <input class="form-control" value="{{$bl->id}}" required type="hidden" name="bl_id[]">
                                        </div>

                                    </td>

                                    <td>
                                        <input class="form-control bl_qty" required  value="{{$bl->bl_qty}}"  type="number" name="qty[]" >
                                    </td>
                                    <td>
                                        <input class="form-control" @if(!$bl->gd_number == null) required @endif value="{{$bl->gd_number}}"  type="text" name="gd_number[]" >
                                    </td>

                                    <td>
                                        @if($key == 0)
                                            <button type="button" id="btn1" value="1" class="btn btn-success">Add BL</button>
                                        @elseif($bl->gd_number == null)
...
compare_qty()
            ...
            $("#btn1").click(function () {
                ...
                $("#acc_table").append("      <tr>\n" +
                    ...
                    "                                                    <input class=\"form-control\" required value='0' type=\"hidden\" name=\"bl_id[]\">\n" +
                    "                                                </div>\n" +
                    "\n" +
                    "                                            </td>\n" +
                    "\n" +
